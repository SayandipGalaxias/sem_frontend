...
export function useAuthApi() {
    ...
    async function login(payload: LoginRequest) {
        ...
        try {
            ...
            if (!useEncryptionStore.getState().randomString) {
                ...
                await useEncryptionStore.getState().saveRandomString(user.email, newRandomString);
            }
    ...
    async function logout() {
        try { await signOutFromDrive(); } catch (_) { }
    ...
    async function register(_email: string, _password: string, _payload: RegisterRequest) {
        setLoading(true);
        try {
